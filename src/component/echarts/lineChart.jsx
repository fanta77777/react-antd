...
class Linechart extends Component {
  ...
  initLine() {
    ...
    console.log("%cechart data", "color:orange", data);
    const {symbolSize} = that.state;
    let myChart = echarts.init(that.lineChart) //初始化echarts
    that.setState({myChart: myChart});
    //我们要定义一个setPieOption函数将data传入option里面
    let options = that.setPieOption(data);
    console.log("%cechart options :", "color:orange", options);
  ...
  showTooltip = (dataIndex) => {
    const {myChart} = this.state;
    myChart.dispatchAction({dataIndex: dataIndex, seriesIndex: 0, type: 'showTip'});
  }
  render() {
    return (<div ref={ref => {
        this.lineChart = ref
